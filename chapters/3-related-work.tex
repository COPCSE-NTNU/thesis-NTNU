...
\end{enumerate}
\todo{Argue why non-maritime topics should be ex- or included. Just be clear why or why not, so ppl know you thought about it.}
...
\begin{itemize}
    ...
    \item \textbf{Data source} - what data source was used in the proposed solution.\todo{If possible, exemplify for each of those (as you did for the first).}
...
These were chosen based on running test queries and evaluating the relevancy and range of the resulting papers. Furthermore, the proposed query had too many Boolean operators for search engines like \textit{ScienceDirect}\footnote{\url{https://sciencedirect.com}}.\todo{Perhaps you can also indicate that there was substantive overlap in the test results for other, non-used databases - if that was indeed the case.}
...
\begin{itemize}
    \item The paper must be published in the last five years. \todo{How sensitive would your filter be to older work. In short: would there be other mechanisms than age that could filter older papers by default (it looks good not dismiss papers purely based on age - similar to not dismiss examiners based on age :) - pun intended)}
