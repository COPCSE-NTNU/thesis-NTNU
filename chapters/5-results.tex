...
In order to further gain insight into the prediction results and the value of the performance metrics, the predictions were further investigated per segment and sub-segment. \cref{fig:segment_accuracy} shows a bar chart of the accuracy of predictions per segment, and it shows that there are not huge differences per segment, however, some vessels of some segments were easier to predict than others.
...
The prediction results for tanker sub-segments show similar results as to the dry bulk ones, however, some other segments does seem to show that size and capacity indeed might be correlated to predictability in different ways. For instance, in the chemical segment this is somewhat the case. The two largest sub-segments have the highest accuracies of \textit{90\%} and \textit{85\%}, yet the remaining sub-segments does not show much difference correlated to size. For the LNG vessels

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/results/seg_chemical_acc}
    \caption{Accuracy of predictions per sub-segment within the ``chemical'' segment.}
    \label{fig:dry_bulk_accuracy}
\end{figure}
